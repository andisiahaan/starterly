<div class="bg-white dark:bg-dark-elevated rounded-lg p-2 md:p-6 w-full">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ $tagId ? __('blog.tags.edit') : __('blog.tags.add') }}</h3>
    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-dark-border">
            ...
            <button type="submit" class="px-4 py-2 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-700 transition">{{ $tagId ? __('common.actions.update') : __('common.actions.create') }}</button>
        </div>
    </form>
</div>
